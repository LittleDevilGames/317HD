...
public class WaveSound {
    ...
    public Buffer getHeader(int i) {
        ...
        buffer.writeInt(0x52494646); // Chunk ID ("RIFF")
        buffer.writeLeInt(36 + subchunk2_size); //Chunk Size
        buffer.writeInt(0x57415645); //Format ("WAVE")

        buffer.writeInt(0x666d7420); //SubChunk1 ID ("fmt ")
        buffer.writeLeInt(16); // SubChunk1 Size
        buffer.writeLeShort(1); // Audio Format (1 for PCM i.e. Linear quantization)
        buffer.writeLeShort(1); // Num Channels(1 = Mono, 2 = Stereo)
        buffer.writeLeInt(22050); // Sample Rate
        buffer.writeLeInt(22050); // Byte Rate ( SampleRate * NumChannels * BitsPerSample / 8 )
        buffer.writeLeShort(1); // Block Align ( NumChannels * BitsPerSample / 8 )
        buffer.writeLeShort(8); // Bits Per Sample

        buffer.writeInt(0x64617461); //SubChunk2 ID ("data")
        buffer.writeLeInt(subchunk2_size); // SubChunk2 Size
    ...
    public void method242(Buffer b) {
        ...
        anInt330 = b.readUnsignedShort();
        anInt331 = b.readUnsignedShort();
    }
