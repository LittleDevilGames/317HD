...
public class SequenceFrame {
    ...
    public static void load(byte[] payload) {
        ...
        int flag_pos = s.readUnsignedShort();
        int mod_pos = s.readUnsignedShort();
        int len_pos = s.readUnsignedShort();
        int skin_pos = s.readUnsignedShort();
        ...
        int count = info_stream.readUnsignedShort();
        ...
        for (int i = 0; i < count; i++) {
            int id = info_stream.readUnsignedShort();

            SequenceFrame a = instance[id] = new SequenceFrame();
            a.length = length_stream.readUnsignedByte();
            a.skinlist = sl;

            int frame_count = info_stream.readUnsignedByte();
            int last_index = -1;
            int frame_index = 0;

            for (int index = 0; index < frame_count; index++) {
                int vertex_mask = flag_stream.readUnsignedByte();
                ...
                if (vertex_mask > 0) {
                    ...
                    if ((vertex_mask & 1) != 0) {
                        vertex_x[frame_index] = modifier_stream.readSmart();
                    } else {
                        vertex_x[frame_index] = vertex;
                    }

                    if ((vertex_mask & 2) != 0) {
                        vertex_y[frame_index] = modifier_stream.readSmart();
                    } else {
                        vertex_y[frame_index] = vertex;
                    }

                    if ((vertex_mask & 4) != 0) {
                        vertex_z[frame_index] = modifier_stream.readSmart();
                    } else {
